...
void TSysDisplayHw::InitReinit()
{
	...
	delay_us(1000);
	AudioInit();
	delay_us(1000); // wait for the start, and give some time to the PWM to charge the LCD circuits

	LcdInit();

	delay_us(1000); // repeat, sometimes at cold start the sound keeps running
	AudioInit();
}
...
void TSysDisplayHw::TurnOnOff(bool aon)
{
	if (aon)
	{
		InitReinit();
	...
	{
		pin_onoff.Set0(); // turns off the display
		pwm_contrast.Disable();

		LcdSetData(0x00); // pull down all data signals
